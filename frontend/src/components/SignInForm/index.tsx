...
export const SignInForm: React.FC<SignInFormProps> = (props) => {
  ...
  const [signInErros, setSignInErrors] = React.useState<String>("");
  ...
  React.useEffect(() => {
    ...
    if (error) setSignInErrors(error);
  }, [isSucsess, error]);

  const onClickSubmit: SubmitHandler<ISignInFormData> = async (data) => {
    const { payload }: any = await dispatch(
      getUser({ username: data.username, password: data.password })
    );

    if (payload) dispatch(setItems(payload.cartItems));
  };

  return (
    <form onSubmit={handleSubmit(onClickSubmit)} className={styles.logInForm}>
      ...
      <section className={styles.registration}>
        ...
        <div className={styles.logIn}>
          <p className={styles.signInError}>
            <small>{signInErros}</small>
          </p>
          Not registered?{" "}
          <em onClick={() => props.setIsLogIn(true)}>
            <u className={styles.logInText}>Log In</u>
          </em>
        </div>
